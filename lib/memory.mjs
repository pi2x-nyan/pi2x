...
export class MemoryStore {
   ...
   */
  async harvest({ session, userId, chatType, targetId, userText, botText, priorIds }) {
    if (!session || !userId) return { ok: false, reason: "no-ctx" };
    if (!userText && !botText) return { ok: false, reason: "empty" };
    // 会话节流
    const row = this.db.prepare("SELECT ts FROM harvest_log WHERE session = ? ORDER BY ts DESC LIMIT 1").get(session);
    const now = Date.now();
    if (row && now - row.ts < this.harvestIntervalMs) return { ok: false, reason: "throttled" };
    ...
    try {
      ...
      const out = await this._extract(userText, botText, priors);
      const facts = Array.isArray(out?.facts) ? out.facts : [];
      // 真实使用信号回写（只有模型明确指认的 id 才算，且必须是本轮真的注入过的）
      ...
      // 来源键统一由 lib/memory-source.mjs 构造（写入与读取必须用同一套规则）。
      // 曾经这里写的是 `${chatType}:${userId}`，群聊时会把**用户QQ当群号**，
      // 与读取端的 `group:<群号>` 对不上 → 群聊收割的记忆全部检索不到。
