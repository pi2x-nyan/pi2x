 ...
 *
 * 【为什么必须存在】
 * 日志里 `[recv] private:123 <昵称>: 注意语气` 这种行，形状与「用户说的话」完全同构。
 * 模型读日志（grep / tail）时，极易把日志行当成用户真实发言，据此凭空推理
 * —— 2026-09-14 实测发生多次（“你说的不行”“这个问题问得关键”等凭空引用）。
 * 给每行加一个固定的 [LOG] 头后，日志与对话在**结构上**就区分开了：
 * 凡以 [LOG] 开头的内容一律是日志，不是用户说的话。
 */
export const LOG_HEAD = "[LOG]";
...
function emit(level, scope, args) {
  ...
  const head = `${LOG_HEAD}${tag}${scopeTag}${turnTag}`;
  // 多行消息：**每一行都带完整头部**。
  // 否则续行（如 `权限: memory.search, …`）在文件里就是一串裸文本，
  // 与对话内容无法区分 —— 这正是模型把日志当成用户发言的成因之一。
  // 续行用 ╎ 标出，便于肉眼快速识别这是同一条日志的延续。
  const body = msg.includes("\n") ? msg.split("\n").map((l, i) => (i === 0 ? l : `╎ ${l}`)).join(`\n${localStamp()} ${head} `) : msg;
  process.stdout.write(`${localStamp()} ${head} ${body}\n`);
}
